...
public class CartesianPlot extends Graph{
	...
	double panelWidth;
	double panelHeight;
	final double PERCENT_MARGIN = 0.05; 	// margin for Catesian plot
	...
	public void setEquaiton(boolean enable) { enableEquation = enable; }
	...
	public void draw(Graphics g) {
        ...
        if (hasXYLables()) {
        	  ...
        	  for (int valueY = intMinY; valueY <= maxY; valueY += getStepYLabel()) {
        	  	  if(valueY != 0) {			 // do not draw 0 might overlap with x	
        	  		  g2.translate(3*pixelWidth,valueY - 3*pixelHeight);
        	  ...
        	  for (int valueX = intMinX; valueX <= maxX; valueX+= getStepXLabel()) {
         		 g2.translate(valueX - 5*pixelWidth,3*pixelHeight);
         		 g2.scale(pixelWidth,-pixelHeight);
		...
		for (int i = 0; i < ds.count(); i++) {
			...
			g2.fill(new Rectangle2D.Double(x - 2*pixelWidth,y - 2*pixelHeight, 4*pixelWidth, 4*pixelHeight) );	
		}
        ...
        if (hasEquation()) {
        	  ...
        	  if (yDraw < 0) 		// not to draw at margin
        		  g2.translate(xMin + 5*pixelWidth,yDraw + 7*pixelHeight );
        	  else
        		  g2.translate(xMin + 5*pixelWidth,yDraw - 7*pixelHeight );
	  		  g2.scale(pixelWidth,-pixelHeight);
