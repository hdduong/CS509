...
public class TestEnableGraphController {

	@Test
	public void testEnableGraphController() {
		Main main = new Main();
		...
		EnableGraphController egc = new EnableGraphController(model);
		egc.process(main, graphOptionPanel);
		
		graphChoosen.setSelectedItem("Column");
		egc = new EnableGraphController(model);
		egc.process(main, graphOptionPanel);
